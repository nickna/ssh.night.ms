...
const leaderboardHotStreaks = `-- name: LeaderboardHotStreaks :many
...
type LeaderboardHotStreaksParams struct {
	PlayedAt pgtype.Timestamptz
	Limit    int32
}

type LeaderboardHotStreaksRow struct {
	Handle   string
	TotalNet int64
}
...
func (q *Queries) LeaderboardHotStreaks(ctx context.Context, arg LeaderboardHotStreaksParams) ([]LeaderboardHotStreaksRow, error) {
	rows, err := q.db.Query(ctx, leaderboardHotStreaks, arg.PlayedAt, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []LeaderboardHotStreaksRow
	for rows.Next() {
		var i LeaderboardHotStreaksRow
		if err := rows.Scan(&i.Handle, &i.TotalNet); err != nil {
			return nil, err
		}
...
const leaderboardLifetimeNet = `-- name: LeaderboardLifetimeNet :many
SELECT u.handle, SUM(r.net)::bigint AS total_net
FROM game_rounds r
JOIN users u ON u.id = r.user_id
GROUP BY u.id, u.handle
...
type LeaderboardLifetimeNetRow struct {
	Handle   string
	TotalNet int64
}
...
// shape on this stack).
func (q *Queries) LeaderboardLifetimeNet(ctx context.Context, limit int32) ([]LeaderboardLifetimeNetRow, error) {
	rows, err := q.db.Query(ctx, leaderboardLifetimeNet, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []LeaderboardLifetimeNetRow
	for rows.Next() {
		var i LeaderboardLifetimeNetRow
		if err := rows.Scan(&i.Handle, &i.TotalNet); err != nil {
			return nil, err
		}
		items = append(items, i)
...
const leaderboardTopSingleWins = `-- name: LeaderboardTopSingleWins :many
...
type LeaderboardTopSingleWinsRow struct {
	Net      int32
	GameKey  string
	PlayedAt pgtype.Timestamptz
	Handle   string
...
func (q *Queries) LeaderboardTopSingleWins(ctx context.Context, limit int32) ([]LeaderboardTopSingleWinsRow, error) {
	rows, err := q.db.Query(ctx, leaderboardTopSingleWins, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []LeaderboardTopSingleWinsRow
	for rows.Next() {
		var i LeaderboardTopSingleWinsRow
		if err := rows.Scan(
			&i.Net,
			&i.GameKey,
			&i.PlayedAt,
