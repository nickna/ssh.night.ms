...
- `internal/security/audit` — structured security-event recorder. Writes synchronously to slog JSON (always; never lost) and asynchronously to the `security_events` Postgres table via a buffered channel + background goroutine (best-effort; drops on overflow with the `audit_events_dropped_total` expvar counter incrementing). Events are concrete structs (`AuthSuccess`, `AuthFailure`, `LockoutHandle`, `LockoutIP`, `PersistentBanAuto`, `PersistentBanManual`, `PersistentBanRevoke`, `ConnRejectedOverlimit`, `HandshakeFailed`) so each event's `details` jsonb payload is well-shaped. The recorder uses `context.Background` internally so a fast-disconnect attacker can't cancel their own audit row. The slog-JSON sink is the durable record; the `security_events` Postgres copy is the queryable hot tier and is bounded by the retention sweeper (below) — it is no longer append-forever.
- `internal/security/retention` — background `Sweeper` (one goroutine, launched in `main.go` next to `banCache.Run`) that bounds `security_events` growth. Runs an initial sweep on boot then every `NIGHTMS_SECURITY_RETENTION_SWEEP_INTERVAL` (default 1h): prunes `severity='info'` (handshake/auth noise) older than `NIGHTMS_SECURITY_EVENT_RETENTION_INFO_DAYS` (default 30) and `severity in ('warn','crit')` older than `NIGHTMS_SECURITY_EVENT_RETENTION_WARN_DAYS` (default 365), and reaps expired `security_ip_bans` rows via the previously-orphaned `DeleteExpiredIPBans`. A `0` retention window disables that tier. Pruned-row count surfaces as the `security_events_pruned_total` expvar. `audit_log` (sysop actions) is intentionally never pruned.
...
**Retention (`internal/security/retention`):** a background `Sweeper` bounds `security_events` so it isn't append-forever. The slog-JSON audit sink remains the durable record; Postgres is only the queryable hot tier, so pruning it is safe.
- `NIGHTMS_SECURITY_RETENTION_SWEEP_INTERVAL` — prune cadence (Go duration). Default `1h`. Hourly small deletes beat one big nightly delete; the work is idempotent.
- `NIGHTMS_SECURITY_EVENT_RETENTION_INFO_DAYS` — retain `severity='info'` events (handshake_failed / auth_failure, ~98% of volume) this long. Default 30. `0` disables info pruning.
- `NIGHTMS_SECURITY_EVENT_RETENTION_WARN_DAYS` — retain `severity in ('warn','crit')` events (lockouts, bans, overlimit) this long. Default 365. `0` disables warn/crit pruning.
- Each tick also reaps expired `security_ip_bans` rows (`DeleteExpiredIPBans`). Pruned `security_events` rows increment the `security_events_pruned_total` expvar. `audit_log` is intentionally never pruned (low volume, high forensic value).
