...
public sealed class ProfileEditScreen : Window
{
    ...
    public ProfileEditScreen(IApplication app, IServiceProvider services, User user)
    {
        ...
        Add(new Label { X = 2, Y = 3, Text = $"Real name (optional, ≤ {ProfileService.MaxRealNameLength} chars):" });
        _realName = new TextField { X = 2, Y = 4, Width = ProfileService.MaxRealNameLength, Text = user.RealName ?? string.Empty };

        Add(new Label { X = 2, Y = 6, Text = $"Location (optional, ≤ {ProfileService.MaxLocationLength} chars):" });
        _location = new TextField { X = 2, Y = 7, Width = ProfileService.MaxLocationLength, Text = user.Location ?? string.Empty };

        Add(new Label { X = 2, Y = 9, Text = $"Bio (optional, ≤ {ProfileService.MaxBioLength} chars):" });
        _bio = new TextView
        {
            X = 2,
            Y = 10,
    ...
    private async Task SaveAsync()
    {
        try
        {
            ...
            if (!result.Ok)
            {
                _app.Invoke(() => _status.Text = $"[!] {result.Error}");
                return;
            }
            ...
            _app.Invoke(() => _status.Text = "Saved.");
        }
        catch (Exception ex)
        {
            _app.Invoke(() => _status.Text = $"[!] save failed: {ex.Message}");
        }
    }
